...
def main(
    ...
    model.load_state_dict(torch.load(pretrained_model_name_or_path, weights_only=True))
    model.eval()
    ...
    for dataset in eval_dataset_list:
        ...
        for video_path, gt_depth_path in zip(video_path_list, gt_depth_path_list):
            ...
            try:
                frames = load_and_preprocess_video(video_path, mode="crop").to(device)
                ...
                with torch.no_grad():
                    ...
                    if gt_depth.shape[-2] != frames.shape[-2] or gt_depth.shape[-1] != frames.shape[-1]:
                        output['depth'] = F.interpolate(output['depth'].squeeze()[:, None, :, :], size=(gt_depth.shape[-2], gt_depth.shape[-1]), mode='bilinear', align_corners=False).squeeze()
