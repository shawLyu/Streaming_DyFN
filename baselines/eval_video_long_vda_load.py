...
@click.option('--pred_depth_dir', type=click.Path(exists=True), default='/home/luban/Video-Depth-Anything/outputs/scannet', help='Path to predicted depth npy files')
@click.option('--gt_depth_dir', type=click.Path(exists=True), default='./demos/video_eval/scannet_long', help='Path to ground truth depth files')
...
@click.option('--target_fps', type=int, default=-1, help='Target frames per second for video processing')
@click.option('--max_res', type=int, default=1280, help='Maximum resolution dimension')
...
def main(
    ...
    for dataset in eval_dataset_list:
        ...
        # Get predicted depth files from pred_depth_dir
        pred_depth_path_list = sorted(glob.glob(str(Path(pred_depth_dir) / "*.npy")))
        # Get GT depth files from gt_depth_dir
        gt_depth_path_list = sorted(glob.glob(str(Path(gt_depth_dir) / "*.npz")))

        print(f"==> Found {len(pred_depth_path_list)} predicted depths and {len(gt_depth_path_list)} gt depths for {dataset}")

        if len(pred_depth_path_list) == 0 or len(gt_depth_path_list) == 0:
            print(f"Warning: No predicted depths or ground truth depths found for {dataset}, skipping...")
            continue

        # Create a mapping from scene name to GT file
        gt_depth_dict = {}
        for gt_path in gt_depth_path_list:
            gt_name = Path(gt_path).stem
            # Extract scene name (e.g., "scene0707_00" from "scene0707_00_disparity")
            scene_name = gt_name.replace('_disparity', '')
            gt_depth_dict[scene_name] = gt_path
        ...
        for pred_depth_path in pred_depth_path_list:
            pred_name = Path(pred_depth_path).stem
            # Extract scene name (e.g., "scene0707_00" from "scene0707_00_rgb_left_depths")
            # Remove "_depths" suffix and any "_rgb_left" suffix
            scene_name = pred_name.replace('_depths', '').replace('_rgb_left', '')
            
            # Find matching GT file
            if scene_name not in gt_depth_dict:
                if not silent:
                    print(f"Warning: No matching GT file found for {pred_name} (scene: {scene_name}), skipping...")
                continue
            
            gt_depth_path = gt_depth_dict[scene_name]
            video_name = scene_name  # Use scene name as video name
            
            if not silent:
                print(f"\n==> Processing {pred_name} and {Path(gt_depth_path).stem}")
            ...
            try:
                ...
                depth = np.load(pred_depth_path)  # Shape: (t, h, w) or (t, 1, h, w)
                
                # Ensure depth has the right shape: (t, 1, h, w)
                if len(depth.shape) == 3:
                    depth = depth[:, None, :, :]
                elif len(depth.shape) == 4 and depth.shape[1] != 1:
                    # If shape is (t, c, h, w) with c != 1, take first channel
                    depth = depth[:, 0:1, :, :]
                
                # Load GT depth
                gt_depth_data = np.load(gt_depth_path, allow_pickle=True)
                gt_depth = gt_depth_data['disparity'] if 'disparity' in gt_depth_data.files else gt_depth_data['arr_0']  # (t, 1, h, w)
                
                gt_depth = np.nan_to_num(gt_depth, nan=0.0, posinf=0.0, neginf=0.0)
                
                # Ensure GT depth has the right shape: (t, 1, h, w)
                if len(gt_depth.shape) == 3:
                    gt_depth = gt_depth[:, None, :, :]
                elif len(gt_depth.shape) == 4 and gt_depth.shape[1] != 1:
                    gt_depth = gt_depth[:, 0:1, :, :]

                # Resize predicted depth to match GT if needed
                if gt_depth.shape[-2] != depth.shape[-2] or gt_depth.shape[-1] != depth.shape[-1]:
                    depth_torch = torch.from_numpy(depth)
                    depth = F.interpolate(depth_torch, size=(gt_depth.shape[-2], gt_depth.shape[-1]), mode='bilinear', align_corners=False).numpy()
                ...
                if save_video:
                    ...
                    # Save the visualized results (only depth, no frames since we don't load video)
