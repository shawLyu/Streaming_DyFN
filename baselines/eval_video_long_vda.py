...
def main(
    ...
    #TODO: Hardcoded checkpoint path temporary
    ...
    for dataset in eval_dataset_list:
        ...
        for video_path, gt_depth_path in zip(video_path_list, gt_depth_path_list):
            ...
            try:
                frames, fps = read_video_frames(video_path, -1, -1, max_res)
                ...
                with torch.no_grad():
                    ...
                    depths, fps = model.infer_video_depth(frames, target_fps, input_size=518, device=device.type, fp32=True)

                    if gt_depth.shape[-2] != frames.shape[-2] or gt_depth.shape[-1] != frames.shape[-1]:
                        depths = F.interpolate(torch.from_numpy(depths[:, None, ...]), size=(gt_depth.shape[-2], gt_depth.shape[-1]), mode='bilinear', align_corners=False).squeeze()
