...
def main(
    ...
    " Here for the debugging purpose "
    gt_depth = np.load("./demos/video_eval/sintel/sleeping_1_disparity.npz")['disparity']

    with torch.no_grad():
        # Use sliding window of size 3 with stride 1
        image_tensor = torch.from_numpy(frames).permute(0, 3, 1, 2).to(device)
        output = model.infer(image_tensor, fov_x=fov_x_, resolution_level=resolution_level, 
        ...
        for i in range(gt_depth.shape[0]):
            pred_depth = torch.nn.functional.interpolate(output['depth'][i][None, None, ...], size=(gt_H, gt_W), 
                            mode='bilinear', align_corners=False)
            scale, shift = align_affine_lstsq(pred_depth.reshape(1, -1), torch.from_numpy(gt_depth[i].squeeze()).to(device).reshape(1, -1), torch.ones_like(pred_depth).reshape(1, -1))
            aligned_depth.append(scale.item() * pred_depth.reshape(1, gt_H, gt_W).cpu().numpy() + shift.item())
            scales.append(scale.item())
            shifts.append(shift.item())

        aligned_depth = np.stack(aligned_depth, axis=0).squeeze()
        aligned_disp = 1 / aligned_depth
        video_name = Path(video_path).stem
        min_disp, max_disp = np.nanquantile(aligned_disp, 0.01), np.nanquantile(aligned_disp, 0.99)
